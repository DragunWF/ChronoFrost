...
def _save_config(master_vol: float, sfx_vol: float) -> None:
    try:
        with open(_CONFIG_PATH, "w", encoding="utf-8") as fh:
            json.dump({"master_vol": master_vol, "sfx_vol": sfx_vol}, fh, indent=2)
    except OSError:
...
class _BgParticle:
    ...
    def _randomise(self, sw: int, sh: int, spread_y: bool) -> None:
        ...
        self.y     = random.uniform(0, sh) if spread_y else sh + random.uniform(0, 40)
...
class MainMenu(BaseScene):
    ...
    def _draw_main(self, screen: pygame.Surface, sw: int, sh: int) -> None:
        ...
        shadow_surf  = self.font_title.render("CHRONOFROST", True, _TITLE_SHADOW)
        ...
        # Subtitle tagline
        sub = self.font_subtitle.render("A BULLET-HELL EXPERIENCE", True, _SUBTITLE)
        screen.blit(sub, (cx - sub.get_width() // 2, title_y + title_surf.get_height() + 10))
