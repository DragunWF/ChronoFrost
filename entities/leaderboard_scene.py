...
_ROW_ODD      = (12, 20, 38)
_ROW_EVEN     = (8, 14, 28)
_RANK_COLOURS = [(255, 210, 50), (195, 195, 210), (180, 115, 55)]  # gold / silver / bronze
...
class LeaderboardScene(BaseScene):
    ...
    def draw(self, screen: pygame.Surface) -> None:
        ...
        for i, entry in enumerate(self._board[:10]):
            ...
            rank_surf  = self.font_row.render(str(i + 1),                         True, rank_col)
            score_surf = self.font_row.render(f"{entry.get('score', 0):,}",       True, rank_col if i < 3 else _TEXT_MAIN)
            date_surf  = self.font_row.render(entry.get("date", "—"),             True, _TEXT_DIM)
            time_surf  = self.font_row.render(entry.get("time", "—"),             True, _TEXT_MAIN)
        ...
        if not self._board:
            empty = self.font_col_hdr.render("-- No runs recorded yet --", True, _TEXT_DIM)
            screen.blit(empty, (cx - empty.get_width() // 2, rows_start + 20))

        # ---- Lifetime stats bar ----
        ls_y = rows_start + 10 * row_h + 18
        pygame.draw.line(screen, _BORDER, (table_x, ls_y), (table_x + table_w, ls_y), 1)
        ls_y += 12
        ...
        # ---- BACK button ----
        if self._button:
            self._button.draw(screen, self.font_btn, self._button.hovered(pygame.mouse.get_pos()))
